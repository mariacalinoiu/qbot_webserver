...
func HandleTests(w http.ResponseWriter, r *http.Request, logger *log.Logger, driver neo4j.Driver, path string, s3Bucket string, s3Profile string) {
	var response []byte
	var status int
	var err error
	...
	case http.MethodPost, http.MethodPut:
		status, err = addTest(r, session, path, s3Bucket, s3Profile, logger)
...
func addTest(r *http.Request, session neo4j.Session, path string, s3Bucket string, s3Profile string, logger *log.Logger) (int, error) {
	token, err := helpers.GetToken(r)
	if err != nil {
	...
	err = datasources.AddTest(session, path, token, test, s3Bucket, s3Profile, logger)
	if err != nil {
