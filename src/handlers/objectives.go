...
func getObjectives(r *http.Request, driver neo4j.Session, path string) ([]byte, int, error) {
	...
	objective, err := datasources.GetObjectives(driver, token, subject)
	if err != nil {
...
func setObjective(r *http.Request, driver neo4j.Session, path string) (int, error) {
	token, err := helpers.GetToken(r)
	if err != nil {
	...
	err = datasources.AddObjective(driver, token, subject, objective)
	if err != nil {
